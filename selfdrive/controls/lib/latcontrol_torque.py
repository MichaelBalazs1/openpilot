...
class LatControlTorque(LatControl):
  ...
  def live_tune(self):
    self.frame += 1
    if self.frame % 300 == 0:
      self._torque_max_lat_accel = float(self.params.get("TorqueMaxLatAccel", encoding="utf8") * 0.01)
      self._torque_friction = float(self.params.get("TorqueFriction", encoding="utf8") * 0.01)
